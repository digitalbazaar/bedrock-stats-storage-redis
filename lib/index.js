...
bedrock.events.on('bedrock.init', () => brPackageManager.register({
  alias: 'redis',
  packageName: 'bedrock-stats-storage-redis',
  type: 'bedrock-stats-store',
}));
...
exports.find = async ({endDate = '+inf', monitorIds, startDate = '-inf'}) => {
  assert.array(monitorIds, 'options.monitorIds');
 ...
 */
exports.getMonitorIds = async () => {
  // smembers returns an empty array if the set does not exist
  return cache.client.smembers(keys.monitorSet());
};
